...
export const uploadFiles = async (req, res) => {
  try {
    ...
    // Only tenant (owner) or admin can upload
    const isAdmin = req.user?.role === "ADMIN" || req.user?.role === "STAFF";
    ...
    fc.tenantSignedAt = new Date();
    fc.status = "WAITING_SIGN";
    await fc.save();

    return res.status(200).json({ success: true, message: "Uploaded signed contract files", data: formatFinalContract(fc) });
  } catch (err) {
...
export const uploadCCCDFile = async (req, res) => {
  try {
    ...
    // Only tenant (owner) can upload CCCD; admin allowed too
    const isAdmin = req.user?.role === "ADMIN" || req.user?.role === "STAFF";
    const isOwnerTenant = fc.tenantId?.toString() === req.user?._id?.toString();
    if (!isAdmin && !isOwnerTenant) return res.status(403).json({ success: false, message: "Forbidden" });
